 ...
 *
 * @param {string} opts.branch='main' The branch to initialize the repository to.
...
export const gitPushFakeCommits = <T extends IGitPushFakeCommits>({
  ...
  return exec(
    ...commits.reduce<ICallable[]>(
      (
        cb,
        { message = 'feat: initial commit', from, tag, branch = 'main' },
      ) => {
        cb.push(
